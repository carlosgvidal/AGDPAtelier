...
async function makeCufflinksManifold(wasm, p) {
  ...
  if(crownWeight>AGDP_SILVER_HOLLOWING.thresholdsGrams.cufflinks.hollowAt*.65){
    visibleCrown=hollowVisibleBody(wasm,visibleCrown,{
      wallMm:AGDP_SILVER_HOLLOWING.conservativeShellWallMm.cufflinks,
      protectedIslandRadiusMm:4.2,
      protectedIslandDepthMm:3.2,
...
// mechanism is attached. The external geometry is unchanged; load-bearing
// hardware remains solid. A central protected island can be retained beneath
// a finding root so the attachment never bears on a thin shell.
function hollowVisibleBody(wasm, body, options){
  const opts=options||{};
  const wall=Math.max(1.05,opts.wallMm||1.20);
  const info=manifoldBounds(body);
  const b=info.b, dim=b.dim;
  if(dim.some(d=>d<wall*2+2.4)) return body;
  const center=[(b.min[0]+b.max[0])/2,(b.min[1]+b.max[1])/2,(b.min[2]+b.max[2])/2];
  const scale=dim.map((d,i)=>clamp((d-2*wall)/d,.18,i===2?.975:.965));
  let cavity=body.translate(center.map(v=>-v)).scale(scale).translate(center);
  if(opts.protectedIslandRadiusMm){
    ...
    const island=cylinderBetween(wasm,
      [center[0],center[1],b.min[2]-wall],
      [center[0],center[1],Math.min(b.max[2],b.min[2]+islandDepth)],
      islandR,64);
    const trimmed=wasm.Manifold.difference(cavity,island);
    try{ cavity.delete(); }catch(e){}
    try{ island.delete(); }catch(e){}
    cavity=trimmed;
  }
  let hollowed;
  try{ hollowed=wasm.Manifold.difference(body,cavity); }
  catch(e){ try{ cavity.delete(); }catch(_){} return body; }
  const holeR=Math.max(1.2,(opts.escapeHoleDiameterMm||2.4)/2);
  const z=b.min[2]+wall*.65;
  const xOff=Math.min(dim[0]*.18,Math.max(wall*2.4,3.0));
  const cutters=[-1,1].map(side=>cylinderBetween(wasm,
    [center[0]+side*xOff,center[1],z-wall*3.0],
    [center[0]+side*xOff,center[1],z+wall*3.0],holeR,32));
  try{
    const perforated=wasm.Manifold.difference(hollowed,unionAll(wasm,cutters));
    const beforeMesh=manifoldToMesh(body);
    const m=manifoldToMesh(perforated);
    const topo=topologyAudit(m.V,m.F);
    const beforeWeight=silverWeightGrams(meshVolumeMm3(beforeMesh.V,beforeMesh.F));
    const afterWeight=silverWeightGrams(meshVolumeMm3(m.V,m.F));
    const reduction=1-afterWeight/Math.max(beforeWeight,1e-6);
    if(!topo.manifoldOK||!Number.isFinite(afterWeight)||reduction<.12||reduction>.88){
      try{ perforated.delete(); }catch(e){}
      return body;
    }
    return perforated;
  }catch(e){ return body; }
...
const AGDP_SILVER_HOLLOWING=Object.freeze({
  ...
  conservativeShellWallMm:Object.freeze({
    // Production walls deliberately remain above the manufacturing floor.
    // 1.20-1.40 mm preserves a materially solid object after casting and
    // polishing while still allowing meaningful internal mass reduction.
    ring:1.22,
    pendant:1.10,
    bangle:1.32,
    cuffBracelet:1.38,
  ...
  thresholdsGrams:Object.freeze({
    ring:Object.freeze({hollowAt:18,rejectAbove:42}),
    pendant:Object.freeze({hollowAt:18,rejectAbove:55}),
    bangle:Object.freeze({hollowAt:58,rejectAbove:125}),
    cuffBracelet:Object.freeze({hollowAt:58,rejectAbove:135}),
    cufflinks:Object.freeze({hollowAt:24,rejectAbove:58}),
    earCuff:Object.freeze({hollowAt:10,rejectAbove:24}),
    brooch:Object.freeze({hollowAt:20,rejectAbove:55}),
    hoopEarring:Object.freeze({hollowAt:Infinity,rejectAbove:24})
  })
});});
...
function applyConservativeSilverHollowing(wasm,manifold,p){
  ...
  if(p.type==='choker'||p.type==='headpiece'){
    throw new Error('AGDP typology removed: '+p.type);
  }
  ...
  const center=[(b.min[0]+b.max[0])/2,(b.min[1]+b.max[1])/2,(b.min[2]+b.max[2])/2];
  // Structural hollowing uses a thicker radial wall for pieces that are
  // expanded, compressed or flexed in use, while removing more material
  // through the low-stress axial faces. This preserves the continuous
  // load path around the wearable arc instead of thinning every axis alike.
  const flexibleBand=p.type==='ring'||p.type==='bangle'||p.type==='cuffBracelet'||p.type==='earCuff';
  const axialWall=flexibleBand?Math.max(1.02,wall*.78):wall;
  const axisWalls=[wall,wall,axialWall];
  const scaleCeiling=flexibleBand?.965:.972;
  const scale=dim.map((d,i)=>clamp((d-2*axisWalls[i])/d,.15,scaleCeiling));
  let inner=manifold.translate(center.map(v=>-v)).scale(scale).translate(center);

  // Open cuffs retain solid terminal zones. These ends are repeatedly
  // opened and closed by the wearer and must not become thin shells.
  if(p.type==='cuffBracelet'||p.type==='earCuff'){
    const openingDeg=clamp(Number(p.opening||70),25,125);
    const arcRad=(360-openingDeg)*Math.PI/180;
    const endpointRadius=Math.max(1.9,wall*2.0);
    const radialCenter=Math.max(Number(p.mainSize||dim[0])*.5+wall*.65,Math.min(dim[0],dim[1])*.36);
    const terminalDepth=Math.max(dim[2]+wall*4,endpointRadius*3);
    const terminalProtectors=[];
    [-arcRad/2,arcRad/2].forEach(a=>{
      const x=center[0]+Math.cos(a)*radialCenter;
      const y=center[1]+Math.sin(a)*radialCenter;
      terminalProtectors.push(cylinderBetween(wasm,[x,y,b.min[2]-wall*2],[x,y,b.max[2]+wall*2],endpointRadius,40));
    });
    try{
      const protectedInner=wasm.Manifold.difference(inner,unionAll(wasm,terminalProtectors));
      try{inner.delete();}catch(e){}
      inner=protectedInner;
    }catch(e){ /* retain the unprotected cavity rather than failing generation */ }
  }

  // Rings retain a solid lower load-bearing sector where impact and grip
  // forces are concentrated. The crown and lateral volumes remain hollow.
  if(p.type==='ring'){
    const guardW=Math.max(dim[0]*.38,wall*7);
    const guardH=Math.max(dim[1]*.20,wall*4.5);
    const guard=wasm.Manifold.cube([guardW,guardH,dim[2]+wall*4],true)
      .translate([center[0],b.min[1]+guardH*.42,center[2]]);
    try{
      const protectedInner=wasm.Manifold.difference(inner,guard);
      try{inner.delete();}catch(e){}
      try{guard.delete();}catch(e){}
      inner=protectedInner;
    }catch(e){}
  }
  ...
  p.silverAxialWallMm=axialWall;
  p.silverStructuralReserve=(p.type==='cuffBracelet'||p.type==='earCuff')?'solid-terminals':(p.type==='ring'?'solid-lower-sector':'continuous-shell');
...
async function makeMeshManifoldEntry(wasm, inputParams){
  ...
  if(p.type==='choker'||p.type==='headpiece'){
    throw new Error('AGDP typology removed from production: '+p.type);
  }
  let manifold;
  if (p.type==='pendant') {
